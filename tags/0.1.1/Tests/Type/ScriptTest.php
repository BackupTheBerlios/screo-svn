...
class ScriptReorganizer_Tests_Type_ScriptTest extends PHPUnit2_Framework_TestCase
{
    ...
    // {{{ public function testPackToScript()
    
    public function testPackToScript()
    {
        $expected = '<?php' . PHP_EOL . PHP_EOL . ( include 'ScriptReorganizer/Tests/files/expectedPackedScript.php' )
            . PHP_EOL . PHP_EOL . '?>';
        $script = new ScriptReorganizer_Type_Script( new ScriptReorganizer_Strategy_Pack );
        $this->target .= 'packedScript.php';
